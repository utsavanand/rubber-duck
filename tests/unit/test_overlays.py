...
from rubberduck.overlays import UVSuiteOverlay, enrich_with_overlay, valid_overlay_session
...
def test_session_meta_reads_announced_session(tmp_path: Path) -> None:
    _write_meta(tmp_path, "abc-123")
    meta = UVSuiteOverlay().session_meta(cwd=tmp_path, overlay_session="abc-123")
    assert meta["name"] == "Entourage Sprint"
    assert meta["priority"] == "high"


def test_session_meta_falls_back_to_current_session_pointer(tmp_path: Path) -> None:
    _write_meta(tmp_path, "abc-123")
    (tmp_path / ".uv-suite-state" / "current-session.txt").write_text("abc-123\n")
    meta = UVSuiteOverlay().session_meta(cwd=tmp_path, overlay_session=None)
    assert meta["name"] == "Entourage Sprint"


def test_session_meta_rejects_traversal_and_garbage(tmp_path: Path) -> None:
    _write_meta(tmp_path, "abc-123")
    ...
    assert UVSuiteOverlay().session_meta(cwd=tmp_path, overlay_session="../../etc/passwd") == {}
    (tmp_path / ".uv-suite-state" / "sessions" / "bad.json").write_text("{not json")
    assert UVSuiteOverlay().session_meta(cwd=tmp_path, overlay_session="bad") == {}
...
def test_enrich_probes_unannounced_claude_sessions(tmp_path: Path) -> None:
    """A session that predates the announcement protocol still gets the overlay
    name via the pointer probe — but only when the runtime matches the
    overlay's base (a codex session in the same cwd is left alone)."""
    _write_meta(tmp_path, "abc-123")
    (tmp_path / ".uv-suite-state" / "current-session.txt").write_text("abc-123")
