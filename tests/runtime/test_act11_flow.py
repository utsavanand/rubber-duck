...
def test_checkpoint_then_rollback_via_server(git_repo: Path, tmp_path: Path) -> None:
    ...
    async def scenario() -> str:
        ...
        async with srv:
            ...
            (wt / "README.md").write_text("good state\n")
            await asyncio.to_thread(_post, port, "/sessions/c/checkpoint", {"label": "good"})
            checkpoints = await asyncio.to_thread(_get, port, "/sessions/c/checkpoints")
            commit = checkpoints["checkpoints"][0]["commit_sha"]  # type: ignore[index]

            (wt / "README.md").write_text("broken\n")
            await asyncio.to_thread(_post, port, "/sessions/c/rollback", {"commit": commit})
            return (wt / "README.md").read_text()

    assert asyncio.run(scenario()) == "good state\n"
